...
export default function Controls(props) {
  ...
  const [showAvialable] = useState([6,7,8,9,12,16,18,21]);
...
return (
    <div className='controls row w-100 ml-0 mt-4 rounded-lg'>
      <div className='col-7 d-flex justify-content-between pt-1 pb-1'>
        ...
        <div className='items mt-0 pt-1' ><span className=' pt-1 d-inline-block' >Show</span> <button className='btn btn-trasparent border ml-2 mt-0 pt-1 mb-0 pb-1 ' onClick={()=> setShow(!showItems)} >{shownum} <AiFillCaretDown /></button> {showItems ? numOfItemsGrid :""}</div>
      </div>
